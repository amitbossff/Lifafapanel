...
app.post('/api/user/create-lifafa', authMiddleware, async (req, res) => {
    try {
        const { title, amount, code, numbers, channel } = req.body;
        const user = req.user;
        ...
        const totalUsers = allowedNumbers.length || 1;
...
app.post('/api/user/claim-lifafa', authMiddleware, async (req, res) => {
    try {
        ...
        if (lifafa.numbers.length > 0 && !lifafa.numbers.includes(user.number)) {
...
app.get('/api/lifafa/:code', async (req, res) => {
    try {
        ...
        let totalUsers = 1; // Default for public lifafa
        if (lifafa.numbers && lifafa.numbers.length > 0) {
            totalUsers = lifafa.numbers.length;
        }
        
        // Calculate remaining users
        const claimedCount = lifafa.claimedCount || 0;
        const remainingUsers = totalUsers - claimedCount;
...
app.post('/api/lifafa/claim', async (req, res) => {
    try {
        ...
        if (lifafa.numbers.length > 0 && !lifafa.numbers.includes(number)) {
            return res.json({ success: false, msg: 'Not eligible' });
        }
        
        if (lifafa.claimedNumbers.includes(number)) {
            return res.json({ success: false, msg: 'Already claimed' });
        }
